import { enlarge, blank } from "./extent.js";
import types from "./types.js";
import { justCoords as geoJsonCoords } from "./geojson.js";

function writePoints(geometries, extent, shpView, shxView, TYPE) {
  var shpI = 0,
    shxI = 0,
    shxOffset = 100;
  ...
  function writePolyLine(coordinates, i) {
    var flattened = geoJsonCoords(coordinates),
...
function shpLength(geometries) {
  return (
    geometries.length * 56 +
    // points
    geoJsonCoords(geometries).length * 16
  );
}

function shxLength(geometries) {
  return geometries.length * 8;
}

function extent(coordinates) {
  return geoJsonCoords(coordinates).reduce(function (extent, c) {
    return enlarge(extent, c);
  }, blank());
}
...
function justCoords(coords, l) {
  ...
  if (typeof coords[0][0] == "object") {
    return coords.reduce(function (memo, c) {
      return memo.concat(geoJsonCoords(c));
    }, l);
  } else {
    return coords;
  }
...
export { writePoints, shpLength, shxLength, extent, parts, totalPoints };
